...
function buildDateRange(days: number) {
  ...
  for (let index = days - 1; index >= 0; index -= 1) {
    ...
    entries.push(cursor.toISOString().slice(0, 10));
  }
  ...
  ensureNodeConfig(payload: {
    ...
    db.prepare(
      `INSERT INTO node_config (
        ...
        use_workspace_model_default, provider, model, credential_id, output_path, temperature, allow_human_input, tool_policy,
  ...
  getProjectRunDetail(projectId: string, runId: string): ProjectRunDetail | null {
    ...
    return {
      ...
      replayHints: {
        ...
        stepRerunReady: true,
        runCompareReady: false,
        notes: "当前版本支持查看调试链路，后续可扩展节点重放与运行对比。",
      },
