...
export const runService = {
  ...
  createReplayRun(input: {
    ...
    ensureRun(input.baselineRunId);
    return evaluationService.createReplayRun(input);
  },
  compareRuns(baselineRunId: string, candidateRunId: string) {
    ensureRun(baselineRunId);
    ensureRun(candidateRunId);
    return { report: evaluationService.compareRuns(baselineRunId, candidateRunId) };
  },
  listEvaluationSuites() {
    return { suites: evaluationService.listSuites() };
  },
  createEvaluationSuite(payload: {
    ...
    return { suite: evaluationService.createSuite(payload) };
  },
  listEvaluationCases(suiteId: string) {
    return { cases: evaluationService.listCases(suiteId) };
  },
  createEvaluationCase(payload: {
    ...
    return { case: evaluationService.createCase(payload) };
  },
  async executeEvaluationCase(caseId: string) {
    return { report: await evaluationService.executeCase(caseId) };
  },
  getEvaluationRun(evaluationRunId: string) {
    return { evaluationRun: evaluationService.getEvaluationRun(evaluationRunId) };
  },
  listEvaluationRuns(limit?: number) {
    return { evaluationRuns: evaluationService.listEvaluationRuns(limit) };
  },
  /**
   * Resume a run that was interrupted by a process crash.
   * Skips nodes already checkpointed as completed and re-executes the rest.
   */
  async resumeRun(runId: string) {
    runtimeEngine.resumeRun(runId).catch((error: unknown) => {
      console.error("[RunService] resumeRun unhandled error:", error);
    });
    return { ok: true };
  },
  /**
   * Scan for and recover all interrupted runs. Call once on server startup.
   */
  async recoverInterruptedRuns() {
    return runtimeEngine.recoverInterruptedRuns();
  },
  getWorkspaceConfig() {
