...
export async function executeDevAgent(input: DevAgentExecutionInput): Promise<DevAgentExecutionResult> {
  const { workspaceId, entryFile, runCommand: rawRunCommand, resolvedInput, env, environmentId, cwdOverride, templateParams } = input;
  ...
  const outputFiles = collectOutputFiles(wsRoot);
