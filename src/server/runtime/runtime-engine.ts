...
class RuntimeEngine {
  ...
  createRun(
    ...
    workflowRef?: { workflowId?: string; workflowVersionId?: string },
  ): Run {
  ...
  private async _startRunImpl(runId: string): Promise<void> {
    ...
    try {
      ...
      await this.executeDagSchedule(runId, dag, new Set(dag.orderedNodeIds), false);
  ...
  sendHumanMessage(
    ...
    const memoryItem = longTermMemoryService.remember({
      scopeType: workflowId ? "workflow" : "workspace",
      scopeId: workflowId ?? "workspace_default",
      runId,
      workflowId,
      nodeId,
  ...
  private async executeNode(runId: string, nodeId: string, rerunMode: boolean) {
    ...
    try {
      ...
      const result = await executionAdapter.run({
        node: freshNode,
        definition,
            ...
            if (type === "llm_request_sent") {
              ...
              if (ptId) {
                memoryStore.updatePromptTrace(ptId, {
                  ...
                  totalTokens: typeof pl?.totalTokens === "number" ? pl.totalTokens : (pTokens + cTokens),
      ...
      memoryStore.updateAgentContext(runId, executionContext.id, (current) => ({
        ...current,
        recentOutputs: [...current.recentOutputs, result.latestOutput].slice(-8),
      ...
      this.emit(runId, "agent_context_updated", {
        ...
        payload: {
          ...
          contextPatch: {
            resolvedInput,
            recentOutputs: [...(executionContext.recentOutputs ?? []), result.latestOutput].slice(-8),
            latestSummary: result.latestOutput,
          },
        },
      });
  ...
  private async executeInputPortNode(runId: string, node: AgentNode, task: Task) {
    ...
    memoryStore.updateAgentContext(runId, context.id, (current) => ({
      ...current,
      recentOutputs: [...current.recentOutputs, dispatchContent].slice(-8),
    ...
    this.emit(runId, "agent_context_updated", {
      ...
      payload: {
        ...
        contextPatch: {
          resolvedInput,
          recentOutputs: [...context.recentOutputs, dispatchContent].slice(-8),
          latestSummary: dispatchContent,
        },
      },
    });
  ...
  private async executeOutputPortNode(runId: string, node: AgentNode, task: Task) {
    ...
    memoryStore.updateAgentContext(runId, context.id, (current) => ({
      ...current,
      recentOutputs: [...current.recentOutputs, finalText].slice(-8),
    ...
    this.emit(runId, "agent_context_updated", {
      ...
      payload: {
        ...
        contextPatch: {
          resolvedInput,
          recentOutputs: [...context.recentOutputs, finalText].slice(-8),
          latestSummary: finalText,
        },
      },
    });
  ...
  private resolveNodeExecutionInput(
    ...
    const inboundText = inbound.length > 0
      ? inbound
          .slice(-12)
    ...
    const humanText = mergedHumanLines.length > 0
      ? mergedHumanLines.map((line) => `- ${line}`).join("\n")
      : "- 无";
    ...
    const memoryQuery = [
      ...
      ...inbound.slice(-8).map((message) => this.describeInboundMessage(message)),
    ...
    const memoryText = memoryHits.length > 0
      ? memoryHits
          .map((item, index) => {
            const header = [
              `#${index + 1}`,
    ...
    return [
      `用户任务:\n${taskTitle}`,
      `节点任务:\n${context.taskBrief || node.taskBrief || "未提供节点任务"}`,
      `上游消息:\n${inboundText}`,
      `人工消息:\n${humanText}`,
      `长期记忆检索:\n${memoryText}`,
      `节点 Prompt:\n${definition.systemPrompt || "无"}`,
    ].join("\n\n");
  }
  ...
  private applyResolvedInput(
    ...
    const contextPatch = {
      ...
      recentOutputs: context.recentOutputs.slice(-8),
    };
  ...
  private async executeDagSchedule(
    ...
    for (const nodeId of dag.orderedNodeIds) {
      ...
      const deps = (dag.incoming.get(nodeId) ?? []).filter((dep) => scope.has(dep)).length;
    ...
    while (readyWave.length > 0) {
      ...
      if (runMode === "standard") {
        await Promise.all(currentWave.map((item) => this.executeNode(runId, item, rerunMode)));
      } else {
  ...
  private async executeDevModeNode(
    ...
    try {
      ...
      memoryStore.updateAgentContext(runId, executionContext.id, (current) => ({
        ...current,
        recentOutputs: [...current.recentOutputs, latestOutput].slice(-8),
      ...
      if (nodeConfig.outputPath) {
        try {
          const outputFilePath = resolve(process.cwd(), nodeConfig.outputPath);
          mkdirSync(dirname(outputFilePath), { recursive: true });
          writeFileSync(outputFilePath, latestOutput, "utf-8");
        } catch { /* non-fatal */ }
      }
